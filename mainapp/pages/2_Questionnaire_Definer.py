...
def run1():
    ...
    #     # Sort columns based on custom criteria
    #     sorted_columns = sorted(updated_df.columns, key=custom_sort)
    #     updated_df = updated_df[sorted_columns]
        
    #     keypress_mappings = {}
    #     drop_cols = []
    #     drop_vals = {}
    #     for col in updated_df.columns[1:-1]:
...
#             if current_question_key and answer_text:
#                 questions_and_answers[current_question_key]['answers'].append(answer_text)
#         else:
#             match = re.match(r"(\d+)\.\s*(.*)", line)
#             if match:
...
#     match = re.match(r"FlowNo_(\d+)=*(\d*)", col)
#     if match:
#         question_num, flow_no = int(match.group(1)), int(match.group(2)) if match.group(2) else 0
#         return (question_num, flow_no)
#     return (float('inf'), 0)
...
#         with col1:
#             st.write("Rename Columns:")
#             new_column_names = ['phonenum']  # Assuming the first column is 'phonenum'
#             for idx, col in enumerate(cleaned_data.columns[1:], start=1):  # Start from 1 to skip 'phonenum'
#                 default_value = st.session_state['qa_dict'].get(f"Q{idx}", {}).get('question', col)
#                 new_name = st.text_input(f"Q{idx}:", value=default_value, key=f"new_name_{idx}")
#                 new_column_names.append(new_name)

#         if st.button("Apply New Column Names"):
#             # Decode keypresses first, using the original column names
#             decoded_df = decode_keypresses(cleaned_data, st.session_state['qa_dict'])
#             st.session_state['decoded_data'] = decoded_df  # Save the decoded DataFrame for further use
#             st.write("DataFrame with Decoded Keypresses:")
#             st.dataframe(decoded_df.head())
            
#             # Then, rename columns in the decoded DataFrame
#             updated_df = rename_columns(decoded_df, new_column_names)
#             st.session_state['renamed_data'] = updated_df  # Save the DataFrame with renamed columns for further use
#             st.write("DataFrame with Decoded Keypresses and Renamed Columns:")
#             st.dataframe(updated_df.head())
